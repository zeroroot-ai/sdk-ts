...
/** One service found in a generated `_pb.ts`. */
interface FoundService {
  /** The exported const, e.g. `HarnessCallbackService`. */
  constName: string
  /** Fully-qualified proto name, e.g. `gibson.harness.v1.HarnessCallbackService`. */
  typeName: string
  /** Import specifier for the SDK's published subpath. */
  importPath: string
  /** Leading comment of the service. */
  description: string
  methods: FoundMethod[]
}

interface FoundMethod {
  /** The generated property name, e.g. `worldView`. Also the client method. */
  localName: string
  description: string
}
 ...
 */
export function cleanComment(block: string): string {
  const lines = block
    .split("\n")
...
export function parseServices(source: string, importPath: string): FoundService[] {
  ...
  for (const m of source.matchAll(SERVICE_RE)) {
    ...
    if (!typeName) throw new Error(`${importPath}: ${constName} has no "@generated from service" comment to read its proto name from`)
    ...
    if (methods.length === 0) throw new Error(`${importPath}: ${constName} parsed with no methods`)
    out.push({
      constName,
      typeName,
      importPath,
...
function emit(services: FoundService[]): string {
...
async function main(): Promise<void> {
  ...
  const services: FoundService[] = []
  for (const file of files) {
    const source = await readFile(file, "utf8")
    if (!SERVICE_RE.test(source)) continue
  ...
  if (services.length === 0) throw new Error(`no services found under ${genDir}; run the SDK's \`pnpm generate\` first`)
